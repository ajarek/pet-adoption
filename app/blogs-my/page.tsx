      ...
      <div>
        {myBlogs?.map((el: any) => {
          return (
            ...
            >
              <Link
                href={`/blog?id=${el._id.toString()}&title=${
                  el.title
                }&description=${el.description}&image=${el.image}&userId=${
                  el.userId
                }`}
                className=" w-full flex items-center gap-4 border-2 border-black p-4"
              >
                <Image
                  src={el.image}
                  alt={el.title}
              ...
              <Link
                href={`/editBlog?id=${el._id.toString()}&title=${
                  el.title
                }&description=${el.description}&image=${el.image}&userId=${
                  el.userId
                }`}
                className="flex items-center justify-center  bg-blue-500 p-2 rounded-sm hover:bg-blue-600"
              >
                <Pencil size={16} />
              </Link>
